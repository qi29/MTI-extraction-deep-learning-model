...
for num in range(1,test_positive_sheet.nrows):
    test_positive_sentences.append(test_positive_sheet.cell(num,1).value)
...
for num in range(0,len(test_X)):
    training_data.append([torch.tensor(numpy.array(test_X[num])), torch.tensor(numpy.array(test_Y[num]))])
...
print ("CWLE_ATDNN test results on dataset2")
