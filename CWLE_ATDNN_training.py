...
for num in range(0,len(positive_sentences_embedding_array)):
    if num < len(positive_sentences_embedding_array)*7/10:
        ...
        test_data.append([torch.tensor(numpy.array(positive_sentences_embedding_array[num])),1])
...
for num in range(0,len(negative_sentences_embedding_array)):
    if num < len(negative_sentences_embedding_array)*7/10:
        ...
        test_data.append([torch.tensor(numpy.array(negative_sentences_embedding_array[num])), 0])
...
print ("CWLE_ATDNN training & test results on dataset1")
check_accuracy(training_data,model)
